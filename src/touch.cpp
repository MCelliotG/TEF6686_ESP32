...
void doTouchEvent(uint16_t x, uint16_t y) {
  ...
  if (scandxmode) {
    ...
    if (!menu && !BWtune && !freqkeypadtune) {
      if (x >= 276 && x < 319 && y >= 1 && y < 30 &&
          batterydetect && !wifi && !screenmute) {
        touchrepeat = false;
        CycleBatteryDisplay();
        return;
      }
         // All pages except menu
    ...
    if (freqkeypadtune) {
      touchrepeat = false;
      int focus = FreqInputHitTest(x, y);
      if (focus >= 0) {
        freqKeyFocus = focus;
        FreqInputAction(freqKeyFocus);
      }
      return;
    }
    ...
    if (!BWtune && !menu && !advancedRDS && !seek && !afscreen) {         // Normal radio mode
      if (x > 0 && x < 320 && y > 180 && y < 240 && band < BAND_GAP) {    // -----------------
        ...
        touchrepeat = false;
        if (tunemode == TUNE_MEM || !freqKeypadEnabled) {
          doBandToggle();
        } else {
          OpenFreqInput(0);
        }
